...
const renderMessage = (status) => {
    ...
    if (status === 'error') markup = '<p>No search results found for your query. Please try again.</p>';
    if (status === 'no-data') markup = '<p>No data for this stock. Please try another stock.</p>';

    searchMessage.insertAdjacentHTML('afterbegin', markup);
}

const generateStockCardMarkup = (currency, symbol, lastestPrice, changeNumber, changePercentage, ma240, ma1200) => {
    return markup = `
        <div class="stock-card">
            <div class="stock-info">
                ...
                <div class="stock-price ${changeNumber > 0 ? 'green' : changeNumber < 0 ? 'red' : ''}">
                    <div class="price">
...
const getAllStocksDataAndRenderCards = async (stocks) => {
    ...
    data.map(stock => {
        ...
        const changePercentage = `${changeIcon}${((change / previousPrice) * 100).toFixed(2)}%`;
        ...
        const markup = generateStockCardMarkup(currency, symbol, lastestPrice, changeNumber, changePercentage, ma240, ma1200);
